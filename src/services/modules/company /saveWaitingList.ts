...
export async function saveWaitingList(waitingListData: FormWaitingListProps, companyId: number, services: string[], user: UserProps, tokenCapTcha: string, employeeId?: string) {
  try {
    ...
    const isUser = await saveServerClient(user, companyId, tokenCapTcha) as UserProps;
